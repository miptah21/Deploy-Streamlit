...
image_file = st.sidebar.file_uploader("Upload an image file", type=["jpg", "png", "tig"])

if image_file is not None:
    input_image = Image.open(image_file)
    
    effect = st.sidebar.selectbox("Select the type of image manipulation.", ['Rotate Image', 'Flip Image', 'Resize Image'])
    ...
    if effect == "Flip Image":
        mirror_choice = st.sidebar.radio("Choose how to  flip the image.", ["Horizontally", "Vertically"])
    ...
    if effect == "Resize Image":
        scale_factor = st.sidebar.slider("Select the precentage size", 5, 10, 6)
        output_image = input_image.resize((input_image.width//scale_factor, input_image.height//scale_factor))
        
    if st.button("Process image"):
        display_image(input_image, output_image)
